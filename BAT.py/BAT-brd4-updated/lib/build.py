...
#       f.write(data)
    ...
    if retain_lig_prot == 'yes':
      ...
      if calc_type == 'dock':
        sp.call('~/.conda/envs/paprika/bin/obabel -i pdb '+pose+'.pdb -o pdb -O '+mol.lower()+'.pdb -d', shell=True)                            # Remove all hydrogens from the ligand
      elif calc_type == 'crystal':
        sp.call('~/.conda/envs/paprika/bin/obabel -i pdb '+mol.lower()+'.pdb -o pdb -O '+mol.lower()+'.pdb -d', shell=True)                     # Remove all hydrogens from crystal ligand
      sp.call('~/.conda/envs/paprika/bin/obabel -i pdb '+mol.lower()+'.pdb -o pdb -O '+mol.lower()+'-h-ini.pdb -p %4.2f' %ligand_ph, shell=True)  # Put all hydrogens back using babel
      sp.call('~/.conda/envs/paprika/bin/obabel -i pdb '+mol.lower()+'.pdb -o mol2 -O '+mol.lower()+'-crg.mol2 -p %4.2f' %ligand_ph, shell=True)
